...
def distance_2d(p1, p2, q1, q2):
    ...
    return math.sqrt( (q1 - p1)**2 + (p2- q2)**2 )
...
def move(speed, distance, is_forward=True):
    ...
    while not rospy.is_shutdown():
        if distance_moved + velocity_msg.linear.x <= distance:
            distance_moved += distance_2d(x, y, x0, y0)
            velocity_publisher.publish(velocity_msg)
        else:
            velocity_msg.linear.x = 0
            velocity_publisher.publish(velocity_msg)

        #rospy.loginfo(velocity_msg.linear.x)
...
def go_to(goal_x, goal_y):
    ...
    while not rospy.is_shutdown():
        distance = distance_2d(goal_x, goal_y, x, y)

        LINEAR_CONSTANT_SPEED = 1.0
        linear_speed = distance * LINEAR_CONSTANT_SPEED
        
        ANGULAR_CONSTANT_SPEED = 5.0
